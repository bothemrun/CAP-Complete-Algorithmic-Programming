...
#define coef(size) ((long long)(1 + size))
#include<functional>
static auto _ = [](){
    ...
    long long minimumCost(int m, int n, vector<int>& horizontalCut, vector<int>& verticalCut) {
        ...
        while(i<hsize && j<vsize){
            if(horizontalCut[i] > verticalCut[j]){
                ans += coef( (j-1) + 1 ) * (long long)horizontalCut[i++];
            }else{
                ans += coef( (i-1) + 1 ) * (long long)verticalCut[j++];
            }
        }

        if(i<hsize){
            ans += coef(vsize) * (long long)accumulate( horizontalCut.begin()+i, horizontalCut.end(), 0 );
        }
        if(j<vsize){
            ans += coef(hsize) * (long long)accumulate( verticalCut.begin()+j, verticalCut.end(), 0 );
        }
        return ans;
    }
};
