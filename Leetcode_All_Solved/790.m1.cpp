...
public:
    int numTilings(int n) {
        const int state_n = 2;
        vector<vector<long long>> dp(3, vector<long long>(state_n));
        //NOTE: use dp[i%3], and store in dp[(i+1)%3]
        dp[ (0+1)%3 ] = {1, 0};
        for(int i=1;i<=n;i++){
            ...
            else dp[ (i+1)%3][0]  = dp[i%3][0];

            dp[ (i+1)%3 ][1] = ( 2 * dp[i%3][0] + dp[i%3][1] )%MOD ;
        }
